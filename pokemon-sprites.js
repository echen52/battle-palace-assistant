// pokemon-sprites.js
// Maps species name -> National Dex number for PokeAPI sprite URLs
// Source: extracted from App.js (Battle Facilities Assistant)
...
/**
 * Returns the sprite <img> element HTML for a given species name,
 * or a placeholder div if the species is not in the dex.
 */
function getSpriteHTML(species) {
  const dex = POKEMON_DEX[species];
  if (dex) {
    return `<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${dex}.png" alt="${species}" onerror="this.style.display='none'" />`;
  }
  return `<div class="no-spr">?</div>`;
}
